...
public class PlacementHandler : MonoBehaviour
{
    ...
    public void DetermineIfSuccesfulClick() // Users click hit an obj
    {
        print(m_cMainCam.transform.position.y);
        RaycastHit _rhCheck = GenerateRayCast(Camera.main.transform.position.y * 2, true);
     //   RaycastHit _rhCheck = GenerateRayCast(205 * 2, false);
        ...
        {
            m_ePlayerState = PlacementHandler.PlayerStates.PLACING;
        }
    ...
    private bool CanPurchase(int _iPurchasing)
    {
        ...
        if (gameObject.GetComponent<PlayerHandler>().m_iPlayerCash >= m_goPossibleObjects[m_iCurrentlyPlacing].GetComponent<ExpenseHandler>().m_iCostToPlace)
        {
            _bAccepted = true;
        }
        return _bAccepted;
